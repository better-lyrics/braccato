// The shapes `fast-xml-parser` produces for a TTML document in `preserveOrder` mode: every element
// becomes `{ <tagName>: [...children], ":@": {...attributes} }`, attributes are prefixed `@_`, and
// namespace prefixes are stripped, so `itunes:key` arrives as `@_key` and `ttm:agent` as `@_agent`.
